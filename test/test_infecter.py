...
def test_find_user_with_most_connections():
	subgraph = CoachingGraph()
	A = User()
	B = User()
	C = User()
	D = User()
	subgraph.add_users([A, B, C, D])
	...
	actual = infecter.find_user_with_most_connections_up_to(subgraph.users, 10)
...
def test_find_user_with_most_connections_up_to():
	graph = CoachingGraph()
	A = User()
	B = User()
	C = User()
	...
	graph.add_users([A, B, C, D, E, F, G, H])
	graph.create_coach_coachee_relationship(A, B)
	graph.create_coach_coachee_relationship(B, D)
	graph.create_coach_coachee_relationship(A, C)
	graph.create_coach_coachee_relationship(A, D)
	graph.create_coach_coachee_relationship(B, C)
	graph.create_coach_coachee_relationship(C, D)

	graph.create_coach_coachee_relationship(E, F)
	graph.create_coach_coachee_relationship(E, G)

	expected = E
	infecter = Infecter()

	actual = infecter.find_user_with_most_connections_up_to(graph.users, 2)
	logger.warning('actual: ' + str(len(actual.neighbors)))
	logger.warning('expected: ' + str(len(expected.neighbors)))
